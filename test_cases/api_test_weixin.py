...
class Test_Weixin:
    ...
    def test_weixin_cookie(self):
        ...
        res_get = requests.get(url=url, params=params)  # 发送请求 get
    ...
    def test_weixin_edit_tag(self):
        ...
        res_post = requests.post(url=url, params=params, json=json)
        json = res_post.json()
        print(json)
    ...
    def test_weixin_files(self):
        ...
        files = {
            "media": open("../img/1.jpg", 'rb')     # 文件上传需要将文件转成二进制进行上传
        }
        res_post = requests.post(url=url, params=params, files=files)
        json = res_post.json()
        print(json)
...
if __name__ == '__main__':
    # 测试获取weixin_cookie值结果
    Test_Weixin().test_weixin_cookie()
    # 测试查询用户标签接口
    Test_Weixin().test_weixin_tag()
    # 测试修改用户标签名接口
    Test_Weixin().test_weixin_edit_tag()
    # 测试上传文件接口
    Test_Weixin().test_weixin_files()